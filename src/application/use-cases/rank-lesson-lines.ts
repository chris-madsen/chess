...
export const rankLessonLines = <T>(candidates: readonly LessonRankCandidate<T>[], limit = 3): readonly LessonRankCandidate<T>[] => (
  [...candidates].sort((first, second) => compareLessonProfiles(first.profile, second.profile)).slice(0, Math.max(1, limit))
);
...
export const chooseShortTalAlternative = <T>(
  ...
  const short = [...baselines].sort((first, second) => first.profile.generatedFullMoves - second.profile.generatedFullMoves || first.profile.lineId.localeCompare(second.profile.lineId))[0];
